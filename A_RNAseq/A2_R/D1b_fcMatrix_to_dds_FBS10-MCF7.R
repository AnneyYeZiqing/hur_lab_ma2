...
codingonly_name <- "LY_FBS10-MCF7_fcCountMatrx_proteinCodingOnly.rds" #Enter desired output file name here
fname_normcounts_human <- "LY_FBS10-MCF7_counts_DDSnormalized.csv" #Enter desired output file name here
ddsrds_name_human <- "LY_FBS10-MCF7_allConds_dds.rds" #This is mainly for use in other R scripts

# Read featureCounts table
fc_filepath <- "/project_rootpath/C1_featureCounts_cmd/LY_FBS10_MCF7_counts.txt"
...
write_deseq_results(res1, paste0(outfilebasepath, "LY_FBS10-MCF7_MA2_vs_mock.csv"))

#2.3.2.  DE diABZI vs mock (DMSO):
res2 <- results(dds, contrast = c("treatment", "diABZI", "DMSO"))
write_deseq_results(res2, paste0(outfilebasepath, "LY_FBS10-MCF7_diABZI_vs_mock.csv"))
