...
codingonly_name <- "LY_FBS10-HCC2429_fcCountMatrx_proteinCodingOnly.rds" #Enter desired output file name here
fname_normcounts_human <- "LY_FBS10-HCC2429_counts_DDSnormalized.csv" #Enter desired output file name here
ddsrds_name_human <- "LY_FBS10-HCC2429_allConds_dds.rds" #This is mainly for use in other R scripts

# Read featureCounts table
fc_filepath <- "/project_rootpath/C1_featureCounts_cmd/LY_HCC2429_counts.txt"
...
treatment <- factor(
  c("diABZI", "diABZI", "MA2", "MA2", "DMSO", "DMSO"),
  levels = c("DMSO", "MA2", "diABZI") #DMSO equals to mock
)
...
write_deseq_results(res1, paste0(outfilebasepath, "LY_FBS10-HCC2429_MA2_vs_mock.csv"))

#2.3.2.  DE diABZI vs mock (DMSO):
res2 <- results(dds, contrast = c("treatment", "diABZI", "DMSO"))
write_deseq_results(res2, paste0(outfilebasepath, "LY_FBS10-HCC2429_diABZI_vs_mock.csv"))
